BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Die_erste_Suedpolarnacht"
TITLE_NAME = "Die erste Südpolarnacht 1898-1899"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Die_erste_Suedpolarnacht_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Die_erste_Suedpolarnacht_*.jpg
	rm cover.jpg 
	rm logo.jpg
